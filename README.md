...
`npm run setup` creates `.env` / `.env.local` with a real `AUTH_SECRET` and applies the database migration.

If demo login still fails: stop the server, run `npm run setup:fresh`, then `npm run dev` again.

### Environment

| Variable | Purpose |
|---|---|
| `AUTH_SECRET` | Required for login — generated by `npm run setup` |
